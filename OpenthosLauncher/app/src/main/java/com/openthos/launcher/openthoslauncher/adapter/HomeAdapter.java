...
public class HomeAdapter extends RecyclerView.Adapter<HomeAdapter.HomeViewHolder> {
    ...
    public HomeViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        HomeViewHolder holder = new HomeViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_icon, parent, false), mRecycleClick);
        return holder;
    }
    ...
    public void onBindViewHolder(HomeViewHolder holder, int position) {
        ...
        if ((int) data.get(position).get("icon") != -1) {
            holder.iv.setImageDrawable(holder.item.getResources().getDrawable((int) data.get(position).get("icon")));
        } else {
